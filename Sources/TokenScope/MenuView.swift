...
struct MenuView: View {
    ...
    @State private var ollamaSetupMessage: String?
    ...
    enum SessionOriginFilter: String, CaseIterable, Identifiable {
        case all, claudeCode, codex, ollamaDesktop, ollamaDirect
        ...
        var label: String {
            ...
            case .ollamaDesktop: return "Desktop"
            case .ollamaDirect: return "Direct"
            }
        }

        func matches(_ session: SessionAgg) -> Bool {
            switch self {
            case .all: return true
            case .claudeCode: return session.provider == .claudeCode || session.surface == .claudeCode
            case .codex: return session.provider == .codex || session.surface == .codex
            case .ollamaDesktop: return session.surface == .ollamaDesktop
            case .ollamaDirect:
                return session.provider == .ollama &&
                    (session.surface == nil || session.surface == .ollamaDirect)
            }
        }

        var markOrigin: UsageOrigin? {
            switch self {
            case .claudeCode: return .claudeCode
            case .codex: return .codex
            case .ollamaDesktop, .ollamaDirect: return .ollama
            case .all: return nil
            }
        }
    }
    ...
    @ViewBuilder private var sessionsContent: some View {
        ...
        let filtered = all.filter(sessionOriginFilter.matches)
        ...
        HStack(spacing: 3) {
            ForEach(SessionOriginFilter.allCases) { filter in
                ...
                Button { sessionOriginFilterRaw = filter.rawValue } label: {
                    HStack(spacing: 4) {
                        if let origin = filter.markOrigin {
    ...
    private func sessionDetail(_ s: SessionAgg) -> String {
        var parts: [String] = []
        if let surface = s.surface { parts.append(surface.displayName) }
        ...
        if s.confidence == .inferred { parts.append("inferred") }
        if s.confidence == .unassigned { parts.append("unassigned") }
    ...
    private var ollamaSourceGroup: some View {
        VStack(alignment: .leading, spacing: 6) {
            sectionTitle("Ollama — local gateway")
            Text("TokenScope meters exact tokens at the local Ollama API boundary. Explicit mode tracks clients pointed at TokenScope; capture-all mode also covers the Ollama Desktop app and normal localhost clients.")
                .font(.system(size: 11)).foregroundStyle(.secondary)
                .fixedSize(horizontal: false, vertical: true)
            HStack(spacing: 6) {
                Circle().fill(store.proxyHealthy ? Color.green : Color.red).frame(width: 7, height: 7)
                Text(store.proxyHealthy
                     ? "\(store.captureAllConfigured ? "Capture all" : "Explicit") · :\(store.proxyPort) → :\(store.upstreamPort)"
                     : "Gateway down")
                    .font(.system(size: 11.5))
                Spacer()
            ...
            HStack(spacing: 6) {
                Image(systemName: store.ollamaDesktopStoreAvailable ? "checkmark.circle.fill" : "minus.circle")
                    .foregroundStyle(store.ollamaDesktopStoreAvailable ? .green : .secondary)
                Text(store.ollamaDesktopStoreAvailable
                     ? "Ollama Desktop session store detected"
                     : "Ollama Desktop session store not detected")
                    .font(.system(size: 10.5)).foregroundStyle(.secondary)
                Spacer()
                Button("Copy capture-all setup") { copyCaptureAllSetup() }
                    .font(.system(size: 11))
            }
            HStack(spacing: 6) {
                Button("Stage capture all") { stageCaptureAll() }
                    .font(.system(size: 11))
                Button("Stage rollback") { stageCaptureRollback() }
                    .font(.system(size: 11))
                if let message = ollamaSetupMessage {
                    Text(message).font(.system(size: 10)).foregroundStyle(.secondary).lineLimit(2)
                }
            }
            Text("Capture-all moves the Ollama daemon to 11435 and lets TokenScope own the standard 11434 endpoint. Port changes take effect after both apps restart.")
                .font(.system(size: 9.5)).foregroundStyle(.secondary)
                .fixedSize(horizontal: false, vertical: true)
        }
    }
    ...
    private func copyCaptureAllSetup() {
        let s = """
        # Quit Ollama and TokenScope first, then run:
        ...
        # Start TokenScope first, then Ollama.
        # Roll back with:
        # launchctl unsetenv OLLAMA_HOST
        # defaults delete com.baysora.tokenscope ProxyPort
        # defaults delete com.baysora.tokenscope OllamaPort
        """
        let pb = NSPasteboard.general
        pb.clearContents()
        pb.setString(s, forType: .string)
    ...
    private func stageCaptureAll() {
        do {
            try OllamaCaptureSetup.stageCaptureAll()
            ollamaSetupMessage = "Staged. Quit both apps; start TokenScope, then Ollama."
        } catch {
            ollamaSetupMessage = "Could not stage: \(error.localizedDescription)"
        }
    }

    private func stageCaptureRollback() {
        do {
            try OllamaCaptureSetup.stageRollback()
            ollamaSetupMessage = "Rollback staged. Restart TokenScope and Ollama."
        } catch {
            ollamaSetupMessage = "Could not roll back: \(error.localizedDescription)"
        }
    }
